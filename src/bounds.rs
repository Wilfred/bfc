...
fn multiply_move_bounds_are_relative() {
    ...
    let instrs = vec![// Move to cell #2.
                      ...
                      MultiplyMove { changes: dest_cells, position: Position { start: 0, end: 0 } }];
...
fn multiply_move_backwards_bounds() {
    ...
    let instrs = vec![PointerIncrement { amount: 1, position: Position { start: 0, end: 0 } }, MultiplyMove { changes: dest_cells, position: Position { start: 0, end: 0 } }];
...
fn excessive_bounds_truncated() {
    ...
    let instrs = vec![PointerIncrement { amount: MAX_CELL_INDEX as isize + 1, position: Position { start: 0, end: 0 } }];
    assert_eq!(highest_cell_index(&instrs), MAX_CELL_INDEX);
}
...
fn increment_offset_bounds() {
    let instrs = [Increment { amount: Wrapping(2), offset: 5, position: Position { start: 0, end: 0 }}];
    assert_eq!(highest_cell_index(&instrs), 5);
}

#[test]
fn set_offset_bounds() {
    let instrs = [Set { amount: Wrapping(2), offset: 10, position: Position { start: 0, end: 0 }},
                  Set { amount: Wrapping(2), offset: 11, position: Position { start: 0, end: 0 }}];
